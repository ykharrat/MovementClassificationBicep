...
function [yUP, featuresTest, Class] = LoadAndAnalyzeUndersample(filename, Class,featuresTest)
load(filename,"datapoints");
load(filename,"feat");
yUP = datapoints(1,2:length(datapoints));
yUP = downsample(yUP,2);
yUP = yUP';
meanV = mean(yUP);
stdV = std(yUP); 
yUP = (yUP - meanV); 
...
hzP10 = bandpower(yUP,100,[1,10]);
hzP20 = bandpower(yUP,100,[10,20]);
hzP30 = bandpower(yUP,100,[20,30]);
hzP50 =  bandpower(yUP,100,[30,49]);
...
firstIndex = 0; 
lastIndex = 0;
endSize = 3; %size of the inactive points for the end of the signals
...
%           plotting = true;
            ...
            % Feature vector
            totalPower = bandpower(yUP(firstIndex:lastIndex,1),100,[1,49]); 
            hzP10 = bandpower(yUP(firstIndex:lastIndex,1),100,[1,10]);
            hzP20 = bandpower(yUP(firstIndex:lastIndex,1),100,[10,20]);
            hzP50 =  bandpower(yUP(firstIndex:lastIndex,1),100,[30,49]);
            % hzP60 =  bandpower(yUP(firstIndex:lastIndex,1),100,[50,60]);
            % hzP100 =  bandpower(yUP(firstIndex:lastIndex,1),100,[60,99]);
            featuresTest = [featuresTest ;lastIndex - firstIndex ,f1, f2, f3, f4, f5, f6,f7, hzP10/totalPower, hzP20/totalPower, hzP30/totalPower, hzP50/totalPower];
            Class = [Class; "UP"];
        end
    end
