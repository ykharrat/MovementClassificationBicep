...
function [yUP, featuresTest, Class] = LoadAndAnalyzeUndersampleByAvg(filename, Class,featuresTest)
load(filename,"datapoints");
load(filename,"feat");
yUP = datapoints(1,2:length(datapoints));
yUPnew = [];
for j = 3:2:(length(yUP)-1)
    yUPnew((j-1)/2) = mean(yUP(j),yUP(j+1));
end
yUP = yUPnew;
yUP = yUP';
meanV = mean(yUP);
...
buffSize = 50; %size of the moving window 
