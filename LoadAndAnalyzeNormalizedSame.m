...
function [yUP, featuresTest, Class] = LoadAndAnalyzeNormalizedSame(filename, Class,featuresTest)
load(filename,"datapoints");
load(filename,"feat");
yUP = datapoints(1,2:length(datapoints));

yUP = yUP';
meanV = 20400; %average among test data and training data
stdV = 4123.6; %maximum std deviation among training data
yUP = (yUP - meanV)./ stdV; 
...
%           plotting = true;
            ...
            % Feature vector
            meanPartition = mean(yUP(firstIndex:lastIndex,1));
            yUP(firstIndex:lastIndex,1) = yUP(firstIndex:lastIndex,1) - meanPartition; % in order to remove DC component
